...
class SecondFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        prefs = requireActivity().getSharedPreferences(getString(R.string.preferences_key_sample), Context.MODE_PRIVATE)

        val ncd = prefs.getInt(getString(R.string.preferences_key3),0)//連続日数
        val MAX : Int = prefs.getInt(getString(R.string.preferences_key_MAX),0)//最長連続日数
        val revival = prefs.getInt(getString(R.string.preferences_key_rev),0)//復活回数
        val totalday =prefs.getInt(getString(R.string.preferences_key2),0)//総日数
        val TT = prefs.getInt(getString(R.string.preferences_key),0)//総時間
